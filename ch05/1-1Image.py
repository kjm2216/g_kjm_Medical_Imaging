...
fig, axs = plt.subplots(1,6, figsize=(12,2)) # 1행 6열의 subplot 생성 / 크기는 12x2

# 각 연산 결과에 대해 heatmapo 그리기
show_heatmap(axs[0],'x',x)  # x 배열에 대한 heatmap
show_heatmap(axs[1],'y',y)  # y 배열에 대한 heatmap
show_heatmap(axs[2],'add by numpy',add)  # NumPy로 더한 결과
show_heatmap(axs[3],'subtract by numpy',subtract)  # NumPy로 뺀 결과
show_heatmap(axs[4],'multiply by numpy', multiply)  # NumPy로 곱한 결과
show_heatmap(axs[5],'divide by numpy', divide)  # NumPy로 나눈 결과 

plt.tight_layout() # 레이아웃을 자동으로 조정하여 그래프가 겹치지 않게 함 
plt.show()
